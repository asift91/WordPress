...
function get_setup_params_from_configs_json
{
    ...
    export wordpressVersion=$(echo $json | jq -r .wordpressProfile.version)
    ...
    export wordpressdbname=$(echo $json | jq -r .wordpressProfile.dbName)
    export wordpressdbuser=$(echo $json | jq -r .wordpressProfile.dbUser)
    export wordpressdbpass=$(echo $json | jq -r .wordpressProfile.dbPassword)
    ...
    export azurewordpressdbuser=$(echo $json | jq -r .wordpressProfile.dbUserAzure)
    export redisDns=$(echo $json | jq -r .wordpressProfile.redisDns)
    export redisAuth=$(echo $json | jq -r .wordpressProfile.redisKey)
    export elasticVm1IP=$(echo $json | jq -r .wordpressProfile.elasticVm1IP)
    export installO365pluginsSwitch=$(echo $json | jq -r .wordpressProfile.installO365pluginsSwitch)
    ...
    export installObjectFsSwitch=$(echo $json | jq -r .wordpressProfile.installObjectFsSwitch)
    export installGdprPluginsSwitch=$(echo $json | jq -r .wordpressProfile.installGdprPluginsSwitch)
    ...
    export searchType=$(echo $json | jq -r .wordpressProfile.searchType)
    export azureSearchKey=$(echo $json | jq -r .wordpressProfile.azureSearchKey)
    export azureSearchNameHost=$(echo $json | jq -r .wordpressProfile.azureSearchNameHost)
    export tikaVmIP=$(echo $json | jq -r .wordpressProfile.tikaVmIP)
    export syslogServer=$(echo $json | jq -r .wordpressProfile.syslogServer)
    export webServerType=$(echo $json | jq -r .wordpressProfile.webServerType)
    ...
    export storageAccountType=$(echo $json | jq -r .wordpressProfile.storageAccountType)
    ...
    export sshUsername=$(echo $json | jq -r .siteProfile.sshUsername)
}
...
function create_wpconfig {
    ...
    cat <<EOF >/wordpress/html/wordpress/wp-config.php
  <?php
  /**
...
function linking_data_location {
    ...
    mkdir -p $dataPath/wordpress
    mkdir -p $dataPath/wordpress/wp-content
    mv /wordpress/html/wordpress/wp-content /tmp/wp-content
    ln -s $dataPath/wordpress/wp-content /wordpress/html/wordpress/
    mv /tmp/wp-content/* $dataPath/wordpress/wp-content/
    chmod 0755 $dataPath/wordpress/wp-content
    chown -R www-data:www-data $dataPath/wordpress
}

function generate_sslcerts {
    local path=/wordpress/certs/
    mkdir -p $path
    ...
    chown -R www-data:www-data /wordpress/data/wordpress
}
...
function create_azure_files_wordpress_share
{
...
function setup_and_mount_gluster_wordpress_share
{
    local glusterNode=$1
    local glusterVolume=$2

    grep -q "/wordpress.*glusterfs" /etc/fstab || echo -e $glusterNode':/'$glusterVolume'   /wordpress         glusterfs       defaults,_netdev,log-level=WARNING,log-file=/var/log/gluster.log 0 0' >> /etc/fstab
    mount /wordpress
}

function setup_and_mount_azure_files_wordpress_share
{
    local storageAccountName=$1
    local storageAccountKey=$2
...
function setup_wordpress_mount_dependency_for_systemd_service
{
  ...
  local systemdSvcOverrideFilePath="${systemdSvcOverrideFileDir}/wordpress_on_azure_override.conf"
...
SERVER_TIMESTAMP_FULLPATH="/wordpress/html/wordpress/.last_modified_time.wordpress_on_azure"
LOCAL_TIMESTAMP_FULLPATH="/var/www/html/wordpress/.last_modified_time.wordpress_on_azure"

# Create a script to sync /wordpress/html/wordpress (gluster/NFS) and /var/www/html/wordpress (local) and set up a minutely cron job
...
if [ -f "$SERVER_TIMESTAMP_FULLPATH" ]; then
  ...
  if [ "\$SERVER_TIMESTAMP" != "\$LOCAL_TIMESTAMP" ]; then
    ...
    rsync -av --delete /wordpress/html/wordpress /var/www/html >> $SYNC_LOG_FULLPATH
  fi
else
...
LAST_MODIFIED_TIME_UPDATE_SCRIPT_FULLPATH="/usr/local/bin/update_last_modified_time.wordpress_on_azure.sh"

# Create a script to modify the last modified timestamp file (/wordpress/html/wordpress/last_modified_time.wordpress_on_azure)
# Should be called by root and only on the controller VM.
# The wordpress admin should run the generated script everytime the /wordpress/html/wordpress directory content is updated (e.g., wordpress upgrade, config change or plugin install/upgrade)
...
# O365 plugins are released only for 'wordpress_xy_STABLE',
# whereas we want to support the wordpress tagged versions (e.g., 'v3.4.2').
# This function helps getting the stable version # (for O365 plugin ver.)
# from a wordpress version tag. This utility function recognizes tag names
# of the form 'vx.y.z' only.
function get_o365plugin_version_from_wordpress_version {
  local wordpressVersion=${1}
  if [[ "$wordpressVersion" =~ v([0-9]+)\.([0-9]+)\.([0-9]+) ]]; then
    echo "WordPress_${BASH_REMATCH[1]}${BASH_REMATCH[2]}_STABLE"
  else
    echo $wordpressVersion
  fi
}

# For wordpress tags (e.g., "v3.4.2"), the unzipped wordpress dir is no longer
# "wordpress-$wordpressVersion", because for tags, it's without "v". That is,
# it's "wordpress-3.4.2". Therefore, we need a separate helper function for that...
function get_wordpress_unzip_dir_from_wordpress_version {
  local wordpressVersion=${1}
  if [[ "$wordpressVersion" =~ v([0-9]+)\.([0-9]+)\.([0-9]+) ]]; then
    echo "wordpress-${BASH_REMATCH[1]}.${BASH_REMATCH[2]}.${BASH_REMATCH[3]}"
  else
    echo "wordpress-$wordpressVersion"
  fi
}

# Long Redis cache wordpress config file generation code moved here
function create_redis_configuration_in_wordpressdata_muc_config_php
{
    # create redis configuration in /wordpress/data/muc/config.php
    cat <<EOF > /wordpress/data/muc/config.php
<?php defined('WordPress_INTERNAL') || die();
 \$configuration = array (
